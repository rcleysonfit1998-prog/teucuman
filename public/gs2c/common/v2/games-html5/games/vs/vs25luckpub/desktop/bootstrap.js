...
var UHT_REVISION = { common: "", desktop: "", mobile: "", uncommon: "" };
var UHT_BUILD_PATH = "";
...
var UHTEventBroker = (function () {
  ...
  function parseOnlineConfig(config) {
    ...
    gameConfig["GAMESERVICE"] = gameConfig["GAMESERVICE"].replace(
      contextPath,
      gsPath,
    );
    ...
    gameConfig["HISTORY"] =
      config["HISTORY"] != null
        ? [config["HISTORY"], magicKeyParam].join(
            /\?/.test(config["HISTORY"]) ? "&" : "?",
          )
        : null;
    gameConfig["REGULATION"] = [config["REGULATION"], magicKeyParam].join(
      /\?/.test(config["REGULATION"]) ? "&" : "?",
    );
    gameConfig["jurisdiction"] = config["jurisdiction"];
    gameConfig["jurisdictionRequirements"] = config["jurisdictionRequirements"];
    gameConfig["LOGOUT"] = [config["LOGOUT"], magicKeyParam].join(
      /\?/.test(config["LOGOUT"]) ? "&" : "?",
    );
    ...
    gameConfig["REGULATION_NOTIFICATION_INTERVAL"] =
      config["REGULATION_NOTIFICATION_INTERVAL"];
    gameConfig["REGULATION_NOTIFICATION_URL"] =
      config["REGULATION_NOTIFICATION_URL"];
    gameConfig["elapsedTime"] = config["elapsedTime"] || config["loggedInTime"];
    gameConfig["selftestUrl"] =
      config["selftestUrl"] || config["RegulationSelfTest"];
    gameConfig["pauseplayUrl"] =
      config["pauseplayUrl"] || config["RegulationSelfExclusion"];
    gameConfig["playlimitUrl"] =
  ...
  function parseOfflineConfig() {
    ...
    gameConfig["ingameLobbyApiURL"] =
      "https://test1.pilot6.pragmaticplay.net/gs2c/minilobby/games";
      ...
      if (xmlhttp.readyState == 4)
        if (xmlhttp.status == 200) {
          ...
          if (
            ...
            window["retriesBeforeAlternativeDatapath"]
          ) {
            url = url.replace(
              UHT_CONFIG.GAME_URL,
            ...
            if (!window["SENT_GA_EVENT_ALTERNATIVE_DATA_PATH"]) {
              ...
              window["globalTracking"]["SendEvent"](
                "uht_loading",
                "_USED_ALTERNATIVE_DATA_PATH",
                1,
                "LoadingTracker",
              );
            }
      ...
      if (progress["currentTarget"]["responseURL"].indexOf("build.js") != -1) {
        if (!window["UHT_CUSTOM_LOADER"])
          document.getElementById("loadingBar").style.width =
            (progress.loaded / UHT_GAME_SIZE) * 70 + 1 + "%";
        UHT_CLIENT_DOWNLOADED =
  ...
  function sendToAdapter(json) {
    ...
    switch (msg) {
      case "EVT_GET_CONFIGURATION":
        sendToGameInternal("EVT_GET_CONFIGURATION", { config: gameConfig });
        return;
      case "EVT_RELOAD":
        location.reload();
        return;
  ...
  function sendToGame(json) {
    ...
    switch (msg) {
      case "EVT_GET_CONFIGURATION":
        ...
        if (config["replayMode"] == true) {
          var url =
            config["replaySystemUrl"] +
            contextPath +
            "/api/replay/data?token=" +
            config["mgckey"] +
            "&roundID=" +
            config["replayRoundId"] +
            "&envID=" +
  ...
  function sendToGameInternal(notification, args) {
    ...
    var params = { common: notification, args: args };
  ...
  function onLoad() {
    ...
    if (isOnline)
      sendToAdapterOriginal(
        JSON.stringify({ common: "EVT_GET_CONFIGURATION", type: "html5" }),
      );
    else {
      parseOfflineConfig();
      loadGame();
...
function UHTVarsInjected() {
  var ProcessGameInfo = function () {
    ...
    UHT_GAME_SIZE += Number(
      window["UHT_SCRIPTS_SIZE"].split(":")[1].replace(",", ""),
    );
    items = [];
  ...
  var SetUHTResources = function () {
    ...
    UHT_RESOURCES = {
      ...
      LOCALIZATIONS:
        loadedFiles.length == numL10nFilesToLoad
          ? loadedFiles.concat(loadedCustomizationFiles)
          : loadedCustomizationFiles,
    };
  };
  ...
  var LoadCustomizationsInfo = function () {
    new PackageLoader(
      UHT_CONFIG.GAME_URL + "customizations.info",
      OnCustomizationsInfoLoaded,
      false,
    );
  };
